...
void AWeapon::EnableCumstomDepth(bool bEnable)
{
	if (WeaponMesh)
	{
...
void AWeapon::OnSphereEndOverlap(UPrimitiveComponent* OVerlappdComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex)
{
	ABlasterCharacter* BlasterCharacter = Cast<ABlasterCharacter>(OtherActor);
	if (BlasterCharacter)
	{
...
void AWeapon::SetWeaponStaete(EWeaponState State)
{
	WeaponState = State;
...
void AWeapon::OnEquipped()
{
	...
	WeaponMesh->SetCollisionEnabled(ECollisionEnabled::NoCollision);
	if (WeaponType == EWeaponType::EWT_SubmachineGune)
	{
...
void AWeapon::Fire(const FVector& HitTarget)
{
	...
	if (CasingClass)
	{
		const USkeletalMeshSocket* AmmoEjectSocekt = WeaponMesh->GetSocketByName(FName("AmmoEject"));
		if (AmmoEjectSocekt)
		{
...
{
	SetWeaponStaete(EWeaponState::EWS_Dropped);
...
FVector AWeapon::TraceEndWithScatter(const FVector& HitTarget)
{
	...
	const FVector ToTargetNomarlized = (HitTarget - TraceStart).GetSafeNormal();
	const FVector SphereCenter = TraceStart + ToTargetNomarlized * DistanceToSphere;
